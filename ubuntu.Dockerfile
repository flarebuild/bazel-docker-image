...
RUN apt update -y
RUN apt install -y --no-install-recommends apt-utils build-essential ca-certificates libssl-dev zlib1g-dev zip unzip
RUN apt install -y --no-install-recommends git golang python-is-python3
